...
// used to render all tiles for preview purpose
function drawTiles () {
    const container = document.getElementById('tiles')
    var content = ''
    tileTypes.forEach((type) => {
        console.log(type)
        const path = drawTile(type, 0, 0, 'white', 'black')
        const tilePreview = `<div>${type}<svg viewbox="0 0 50 50" class="h-auto w-full"><g fill="none">${path}</g></svg></div>`
        content += tilePreview
    })
